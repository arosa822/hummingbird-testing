...
- **Registry:** quay.io/hummingbird/postgresql
- **Variants:** 
  - `latest` - Default distroless variant
  - `latest-builder` - Builder variant with package manager

## Description
PostgreSQL advanced open-source relational database system with strong SQL compliance.

## Primary Utility
postgres server for SQL database operations
...
### Pull and Inspect
```bash
podman pull quay.io/hummingbird/postgresql:latest
podman inspect quay.io/hummingbird/postgresql:latest
```
...
### Security Scan
```bash
./ci/grype-hummingbird.sh quay.io/hummingbird/postgresql:latest
```
