...
## Image Information
- **Image Name:** caddy
- **Registry:** quay.io/hummingbird/caddy
- **Variants:** 
  - `latest` - Default distroless variant
  - `latest-builder` - Builder variant with package manager

## Description
Caddy web server with automatic HTTPS, HTTP/2, and modern web server features.

## Primary Utility
caddy command for serving HTTP/HTTPS traffic
...
podman inspect quay.io/hummingbird/caddy:latest
```
...
### Security Scan
```bash
./ci/grype-hummingbird.sh quay.io/hummingbird/caddy:latest
```
