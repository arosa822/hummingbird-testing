...
## Image Information
- **Image Name:** php
- **Registry:** quay.io/hummingbird/php
- **Variants:** 
  - `latest` - Default distroless variant
  - `latest-builder` - Builder variant with package manager

## Description
PHP programming language runtime for executing PHP web applications and scripts.

## Primary Utility
php command for running PHP scripts and applications
...
### Pull and Inspect
```bash
podman pull quay.io/hummingbird/php:latest
podman inspect quay.io/hummingbird/php:latest
```
...
### Security Scan
```bash
./ci/grype-hummingbird.sh quay.io/hummingbird/php:latest
```
