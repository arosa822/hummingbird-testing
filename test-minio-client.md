# Test Plan: minio-client

## Image Information
- **Image Name:** minio-client
- **Registry:** quay.io/hummingbird/minio-client
- **Variants:** 
  - `latest` - Default distroless variant
  - `latest-builder` - Builder variant with package manager

## Description
MinIO Client (mc) command-line tool for interacting with S3-compatible object storage.

## Primary Utility
mc command for object storage operations
...
### Pull and Inspect
```bash
podman pull quay.io/hummingbird/minio-client:latest
podman inspect quay.io/hummingbird/minio-client:latest
```
...
### Security Scan
```bash
./ci/grype-hummingbird.sh quay.io/hummingbird/minio-client:latest
```
