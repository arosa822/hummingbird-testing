...
echo "=================================="
echo "Phase 1: go-1-25 Image Tests"
echo "=================================="
echo ""

# Test 1: Verify Go version
echo "[TEST 1] Verify Go 1.25 version"
echo "Command: ${TEST_ENGINE} run --rm ${IMAGE} go version"
VERSION_OUTPUT=$(${TEST_ENGINE} run --rm ${IMAGE} go version)
echo "Output: ${VERSION_OUTPUT}"
if [[ "$VERSION_OUTPUT" == *"go1.25"* ]]; then
    echo "✓ PASSED - Go 1.25 detected"
else
    echo "✗ FAILED - Expected Go 1.25, got ${VERSION_OUTPUT}"
    exit 1
fi
echo ""
...
# Test 4: Verify Go toolchain (build command)
...
BUILD_OUTPUT=$(${TEST_ENGINE} run --rm -v "${SCRIPT_DIR}/test_program.go:/tmp/test_program.go:ro,z" ${IMAGE} go build -o /tmp/test_binary /tmp/test_program.go 2>&1)
if [ $? -eq 0 ]; then
