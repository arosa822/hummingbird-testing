...
echo "=================================="
echo "Phase 1: python-3-14 Image Tests"
echo "=================================="
echo ""

# Test 1: Verify Python version
echo "[TEST 1] Verify Python 3.14 version"
echo "Command: ${TEST_ENGINE} run --rm ${IMAGE} python3 --version"
VERSION_OUTPUT=$(${TEST_ENGINE} run --rm ${IMAGE} python3 --version)
echo "Output: ${VERSION_OUTPUT}"
if [[ "$VERSION_OUTPUT" == *"Python 3.14"* ]]; then
    echo "✓ PASSED - Python 3.14 detected"
else
    echo "✗ FAILED - Expected Python 3.14, got ${VERSION_OUTPUT}"
    exit 1
fi
echo ""
...
echo "Command: ${TEST_ENGINE} run --rm -v ${SCRIPT_DIR}/test_script.py:/app/test.py:ro,z ${IMAGE} python3 /app/test.py"
SCRIPT_OUTPUT=$(${TEST_ENGINE} run --rm -v "${SCRIPT_DIR}/test_script.py:/app/test.py:ro,z" ${IMAGE} python3 /app/test.py)
...
echo "=================================="
echo "All python-3-14 tests PASSED!"
echo "=================================="
