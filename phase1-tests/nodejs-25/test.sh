...
echo "=================================="
echo "Phase 1: nodejs-25 Image Tests"
echo "=================================="
echo ""

# Test 1: Verify Node.js version
echo "[TEST 1] Verify Node.js 25.x version"
echo "Command: ${TEST_ENGINE} run --rm ${IMAGE} --version"
VERSION_OUTPUT=$(${TEST_ENGINE} run --rm ${IMAGE} --version)
echo "Output: ${VERSION_OUTPUT}"
if [[ "$VERSION_OUTPUT" == *"v25."* ]]; then
    echo "✓ PASSED - Node.js 25.x detected"
else
    echo "✗ FAILED - Expected Node.js v25.x, got ${VERSION_OUTPUT}"
    exit 1
fi
echo ""
...
echo "=================================="
echo "All nodejs-25 tests PASSED!"
echo "=================================="
